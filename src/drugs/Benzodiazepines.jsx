...
function GabaTip({ active, payload, label }) {
...
function Overview({ desk }) {
  return (
    <div>
      <SectionTitle color={COLOR}>💊 Benzodiazépines — dépendance médicale</SectionTitle>
...
function Timeline({ desk }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 11, color: COLOR, letterSpacing: 3, fontFamily: "monospace", marginBottom: 16, paddingBottom: 8, borderBottom: `1px solid ${COLOR}22` }}>PARTIE 1 — EFFETS ACTIFS (sensibilité GABA-A)</div>
      ...
      <div style={{ marginBottom: 32 }}>
        <Card color={COLOR}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={gabaData} margin={{ top: 10, right: 10, bottom: 5, left: -20 }}>
              <defs>
              ...
              <Area type="monotone" dataKey="sensibilite" stroke={COLOR} fill="url(#benz-gabaGrad)" strokeWidth={2} name="Sensibilité GABA-A" dot={false} />
              <Area type="monotone" dataKey="drug" stroke="#818cf8" fill="url(#benz-drugGrad)" strokeWidth={2} name="Benzos actifs" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
      </div>

      <div style={{ fontSize: 11, color: "#22c55e", letterSpacing: 3, fontFamily: "monospace", marginBottom: 16, marginTop: 8, paddingBottom: 8, borderBottom: "1px solid #22c55e22" }}>PARTIE 2 — ARRÊT & RÉCUPÉRATION (sevrage)</div>

      <div style={{ marginBottom: 32 }}>
        <Card color="#ef4444">
          <div style={{ fontSize: 11, color: "#ef4444", letterSpacing: 2, fontFamily: "monospace", marginBottom: 6 }}>CORTISOL — SUPPRESSION ET REBOND EN SEVRAGE (µg/dL)</div>
          <div style={{ fontSize: 11, color: "#475569", marginBottom: 16, lineHeight: 1.5 }}>Sous benzos : cortisol supprimé. Arrêt : spike massif (×4 pic à J3). Avec taper progressif : remontée contrôlée.</div>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={cortisolData} margin={{ top: 10, right: 10, bottom: 5, left: -20 }}>
              ...
              <Area type="monotone" dataKey="drug" stroke="#ef4444" fill="url(#benz-cortDrug)" strokeWidth={2} name="Sevrage benzos" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
      </div>

      <div style={{ fontSize: 11, color: "#475569", letterSpacing: 2, fontFamily: "monospace", marginBottom: 14 }}>PROTOCOLE DE TAPER (RÉDUCTION PROGRESSIVE)</div>
      <div style={{ display: "grid", gridTemplateColumns: desk ? "1fr 1fr" : "1fr", gap: 10 }}>
        {[
          ...
          <div key={i} style={{ background: "#0d0d1a", border: "1px solid #1e1e3a", borderRadius: 10, padding: 12 }}>
            <div style={{ fontSize: 13, color: item.color, fontWeight: "bold", marginBottom: 4 }}>{item.step}</div>
...
function Neurochimie({ desk }) {
  return (
    <div>
      <SectionTitle color={COLOR}>Neurochimie — GABA & hormones</SectionTitle>
...
function Safety({ desk }) {
  return (
    <div>
      <SectionTitle color={COLOR}>Sécurité — sevrage progressif & ressources</SectionTitle>
...
const NAV = [
  ...
  { id: "risks", label: "Dépendance médicale" },
...
function Sources({ desk }) {
  const refs = [
    ...
    { authors: "Hessler-Jones D et al.", title: "Synaptic correlates of benzodiazepine tolerance", journal: "Nature — Molecular Psychiatry", year: "2026", url: "https://www.nature.com/articles/s41380-026-03512-8" },
    { authors: "Tietz EI et al.", title: "Inhibitory and excitatory synaptic neuroadaptations in the diazepam tolerant brain", journal: "PMC / Pharmacology Biochemistry and Behavior", year: "2023", url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC10578451/" },
    { authors: "Griffin CE et al.", title: "Benzodiazepine pharmacology and central nervous system-mediated effects", journal: "PMC / The Ochsner Journal", year: "2013", url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC9879605/" },
    { authors: "ASAM", title: "ASAM Clinical Practice Guideline on Benzodiazepine Taper Management", journal: "American Society of Addiction Medicine", year: "2025", url: "https://www.asam.org/quality-care/clinical-guidelines/benzodiazepines" },
  ];
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ background: `${COLOR}10`, border: `1px solid ${COLOR}33`, borderRadius: 12, padding: 20 }}>
        ...
        <p style={{ color: "#94a3b8", fontSize: 13, lineHeight: 1.7, margin: 0 }}>
          Toutes les données présentées sont issues de publications scientifiques révisées par les pairs (2010–2026).
      ...
      <div style={{
        ...
        <div style={{ position: "relative", maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {[
              { label: "GABA-A modulateur", color: COLOR },
              { label: "Dépendance 2-4 sem", color: "#ef4444" },
              { label: "Taper obligatoire", color: "#fbbf24" },
              { label: "Prescription médicale", color: "#818cf8" },
            ].map((b, i) => (
